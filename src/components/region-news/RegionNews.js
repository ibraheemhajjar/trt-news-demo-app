...
const RegionNews = () => {
     ...
     const fetchRegionNews = async () => {
          try {
               ...
               const startIndex = Math.abs((Math.floor(Math.random() * newsFeed?.data?.length)) - 3)
               ...
               {regionNewsFeed.map((newsItem, index) => {
                    return (
                         <div className="region-news-item-container" key={index}>
                              <Link to={newsItem?.path} className="region-item-container hover-action">
                                   <h4 className="region-news-tag">{newsItem?.categories[1].title}</h4>
